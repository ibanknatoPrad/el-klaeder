...
function draw() {
  ...
  ctx.rotate(3.142857142857143 / 2);
  ctx.fillText("heAT", 0, 0);
  ctx.restore();

  for (let i = 0; i < particles.length; i++) {
    let particle = particles[i];
    ctx.beginPath();
    ctx.arc(particle.x, particle.y, particle.radius, 0, 3.142857142857143 * 2);
    ctx.fillStyle = getGradient(
...
function drawArc(x, y, radius, lineWidth, startColor, endColor) {
  ctx.beginPath();
  ctx.arc(x, y, radius, 0, 3.142857142857143 * 2);
